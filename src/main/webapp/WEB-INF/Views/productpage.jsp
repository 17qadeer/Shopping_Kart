...
<div class="container">
    ...
    <div class="details">
        <div class="content">
            ...
            <c:forEach items="${product.productColor}" var="color">
                <span class="${color.color}" data-color-primary="${color.primary_color}" data-color-sec="${color.secondary_color}" data-pic="${color.url}"></span>
               </c:forEach>
                </p>
            <h3>${product.price}</h3>
            <br><br><h4>Size:<c:forEach items="${product.productSize}" var="size">
            <td>${size.size}</td>
    </c:forEach></h4>
            <form action="/add" method="post">
    <input type="hidden" name="productVariationId" value="${product.id}"> <!-- productId value should be dynamically populated -->
